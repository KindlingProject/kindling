...
#include "src/userspace/libscap/scap_savefile.h"
...
static const char *HCMINE_BPF_PROBE_ENV = "HCMINE_BPF_PROBE";
static const char *HCMINE_FILTER_IFNAME_ENV = "HCMINE_FILTER_IFNAME";
			   ...
			   const char **suppressed_comms)
{
	...
	if(bpf_probe)
	{
		...
		if(strlen(bpf_probe) == 0)
		{
			...
			snprintf(buf, sizeof(buf), "%s/.hcmine/%s-bpf.o", home, PROBE_NAME);
			bpf_probe = buf;
		}
	}
	else
...
int32_t scap_fill_eventmask(scap_t* handle) {
#if !defined(HAS_CAPTURE) || defined(CYGWING_AGENT)
	snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE, "eventmask not supported on %s", PLATFORM_NAME);
	return SCAP_FAILURE;
#else
	return(scap_handle_eventmask(handle, PPM_IOCTL_MASK_FILL_EVENTS, 0));
#endif
}
...
int32_t scap_enable_skb_capture(scap_t *handle)
{
	...
	if(handle->m_bpf)
	{
		const char *ifname = getenv(HCMINE_FILTER_IFNAME_ENV);
		if(ifname == NULL)
			ifname = "lo";
...
{
	char* p = getenv("HCMINE_HOST_ROOT");
...
{
	return getenv(HCMINE_BPF_PROBE_ENV);
}
