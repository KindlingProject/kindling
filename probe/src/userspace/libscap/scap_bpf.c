...
#include "src/userspace/libscap/scap-int.h"
#include "src/userspace/libscap/scap_bpf.h"
#include "src/userspace/libscap/scap_symbol.h"
#include "src/driver/driver_config.h"
#include "src/driver/bpf/types.h"
#include "src/userspace/libscap/compat/misc.h"
#include "src/userspace/libscap/compat/bpf.h"
...
int32_t load_tracepoint(scap_t* handle, const char *event, struct bpf_insn *prog, int size)
{
	...
	else if(is_uprobe || is_uretprobe)
	{
		...
		if(memcmp(event, "filler/", sizeof("filler/") - 1) != 0){
			uint64_t addr;
			err = resolve_symbol_name("/usr/lib64/libpthread-2.17.so", event, &addr);
			if (err < 0){
			...
			snprintf(buf, sizeof(buf), "%s%s %s:0x%"PRIx64"",
					 is_uprobe ? "p:p_" : "r:r_", event, "/usr/lib64/libpthread-2.17.so", addr);
...
static int32_t load_bpf_file(scap_t *handle, const char *path)
{
	...
	for(j = 0; j < ehdr.e_shnum; ++j)
	{
		...
		if(memcmp(shname, "tracepoint/", sizeof("tracepoint/") - 1) == 0 ||
		   ...
		   memcmp(shname, "uprobe/", sizeof("uprobe/") - 1) == 0 ||
		   memcmp(shname, "uretprobe/", sizeof("uprobe/") - 1) == 0)
		{
...
int32_t scap_bpf_clear_eventmask(scap_t* handle)
{
    ...
    memset(settings.events_mask, 0, sizeof(settings.events_mask));
...
static int32_t set_default_settings(scap_t *handle)
{
	...
	for (int i = 0; i < PPM_EVENT_MAX; i++) {
	    settings.events_mask[i] = true; // default collect all
	}
//    memset(settings.events_mask, 0, sizeof(settings.events_mask)); // default collect nothing
