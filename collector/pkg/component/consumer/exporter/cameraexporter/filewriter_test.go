...
func TestWriteTrace(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		...
		for j := 0; j < 10; j++ {
			writer.writeCpuEvents(cpuEvent(int64(1660790510000000000+j*1e9), triggerKey(traceData)))
		}
	}
	filesName, err := getFilesName(filepath.Join(fileConfig.StoragePath, strconv.FormatInt(int64(pid), 10)))
...
func cpuEvent(startTime int64, triggerKey string) *model.DataGroup {
	labels := model.NewAttributeMap()
	...
	labels.AddStringValue("trigger_key", triggerKey)
