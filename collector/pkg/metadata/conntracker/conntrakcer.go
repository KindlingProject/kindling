...
import (
	...
	internal2 "github.com/Kindling-project/kindling/collector/pkg/metadata/conntracker/internal"
...
func NewConntracker(config *Config) (Conntracker, error) {
	once.Do(func() {
		if config == nil || !config.Enabled {
			...
			conntracker, err := internal2.NewConntracker(cfg)
			if err != nil {
...
type NetlinkConntracker struct {
	conntracker internal2.Conntracker
	cfg         *Config
}
...
func (ctr *NetlinkConntracker) GetDNATTupleWithString(srcIP string, dstIP string, srcPort uint16, dstPort uint16, isUdp uint32) *IPTranslation {
	conn := internal2.ConnectionStats{
		Source: net.ParseIP(srcIP),
		SPort:  srcPort,
		Dest:   net.ParseIP(dstIP),
		DPort:  dstPort,
		Type:   internal2.ConnectionType(isUdp),
	}
...
func (ctr *NetlinkConntracker) getDNATTuple(conn internal2.ConnectionStats) *IPTranslation {
